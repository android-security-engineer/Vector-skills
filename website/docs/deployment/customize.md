# 🎨 站点定制

VitePress 文档站的视觉与功能定制：主题色、logo、mermaid 配色、自定义域名、footer。

> 📂 [`website/docs/.vitepress/theme/custom.css`](https://github.com/android-security-engineer/Vector-skills/blob/master/website/docs/.vitepress/theme/custom.css) · `config.ts`
> 🚀 deployment 运维

## 定制入口

| 定制项 | 文件 | 说明 |
| :--- | :--- | :--- |
| 主题色 | `theme/custom.css` | `--vp-c-brand-*` 变量 |
| 字体 | `theme/custom.css` | `--vp-font-family-base/mono` |
| logo / favicon | `config.ts` + `public/` | `logo`、`head` link |
| mermaid 配色 | `config.ts` `mermaidConfig` | 全局图表样式 |
| 自定义组件 | `theme/index.ts` | `app.component` 全局注册 |
| 自定义域名 | GitHub Pages 设置 | CNAME |
| footer | `config.ts` `themeConfig.footer` | 版权信息 |

## 定制链路全景

从源码到上线，定制项分布在三个层：构建配置 → 主题资源 → 部署平台。下图把各定制点与其落点文件一次讲清：

```mermaid
graph LR
    subgraph 源码层["源码层（仓库内）"]
        CFG["config.ts<br/>站点元数据 / nav / mermaid"]:::vector
        CSS["theme/custom.css<br/>品牌色 / 字体"]:::vector
        IDX["theme/index.ts<br/>全局组件注册"]:::vector
        PUB["public/<br/>favicon / CNAME"]:::vector
    end
    subgraph 构建层["构建层（VitePress）"]
        BUILD["vitepress build<br/>注入 base / themeConfig"]:::gray
    end
    subgraph 部署层["部署层（GitHub Pages）"]
        PAGES["Pages 分支<br/>静态站点产物"]:::blue
        DOMAIN["自定义域名<br/>CNAME + base='/'"]:::warn
    end
    CFG --> BUILD
    CSS --> BUILD
    IDX --> BUILD
    PUB --> BUILD
    BUILD --> PAGES
    PAGES -.可选.-> DOMAIN
    classDef vector fill:#0e3a36,stroke:#3dd8c8,color:#bff5ec
    classDef gray fill:#1a2030,stroke:#6b7689,color:#c9cedd
    classDef blue fill:#143a4a,stroke:#4fb3d8,color:#bff0f5
    classDef warn fill:#3a2a10,stroke:#e8a838,color:#ffd9b0
```

- **源码层**全部位于 [`website/docs/.vitepress/`](https://github.com/android-security-engineer/Vector-skills/blob/master/website/docs/.vitepress/) 目录；
- **构建层**由 [`website/package.json`](https://github.com/android-security-engineer/Vector-skills/blob/master/website/package.json) 的 `build` 脚本驱动；
- **部署层**由 `.github/workflows/` 下的 Pages workflow 推送产物到 `gh-pages`。

## 主题色

`custom.css` 定义浅色与深色两套品牌色，锚定"近黑冷蓝 + 电光青"：

```css
:root {
  --vp-c-brand-1: #0e8c7e;   /* 浅色主色 */
  --vp-c-brand-2: #0a7468;
  --vp-c-brand-3: #14b3a0;
  --vp-c-brand-soft: rgba(14, 140, 126, 0.14);
}
.dark {
  --vp-c-brand-1: #3dd8c8;   /* 深色电光青 */
  --vp-c-brand-2: #4fe6d8;
  --vp-c-brand-3: #6ff0e4;
}
```

修改这些变量即可全局换色。`--vp-c-indigo-*` 同步设为青色，使 VitePress 内置组件（如按钮、徽章）一致。

## mermaid 配色

文档中 mermaid 图采用统一 classDef 配色约定：

| classDef | fill / stroke | 含义 |
| :--- | :--- | :--- |
| Vector 青 | `#0e3a36` / `#3dd8c8` | Vector 自身组件 |
| 琥珀 | `#3a2a10` / `#e8a838` | 问题/警告/检查 |
| 灰蓝 | `#1a2030` / `#6b7689` | 普通节点 |
| 绿 | `#1a3a1a` / `#5cd980` | 成功/通过 |
| 蓝 UI | `#143a4a` / `#4fb3d8` | UI/系统侧 |

`config.ts` 的 `mermaidConfig` 设全局字体与布局：

```ts
const mermaidConfig = {
  theme: { variables: {
    fontFamily: '"JetBrains Mono", "Fira Code", ui-monospace, monospace',
    fontSize: '14px',
  }},
  flowchart: { curve: 'basis', htmlLabels: true, useMaxWidth: true },
}
```

### mermaid 渲染链路

markdown 中的 ```` ```mermaid ```` 代码块在浏览器侧由 `v-mermaid` 指令实时渲染，配色通过 `classDef`/`style` 内联控制。下图说明一条 mermaid 块从书写到上屏的全链路：

```mermaid
graph LR
    MD["markdown 源文件<br/>```mermaid 块```"]:::vector
    MD -->|vitepress 编译| VUE[".vue 页面<br/>注入 MermaidPlugin"]:::vector
    VUE -->|浏览器运行时| MM["mermaid.js<br/>解析 DSL → SVG"]:::gray
    MM -->|classDef/style| STYLE["应用配色<br/>青/琥珀/灰蓝/绿/蓝"]:::vector
    STYLE --> SVG["内联 SVG<br/>随主题切换"]:::good
    classDef vector fill:#0e3a36,stroke:#3dd8c8,color:#bff5ec
    classDef gray fill:#1a2030,stroke:#6b7689,color:#c9cedd
    classDef good fill:#1a3a1a,stroke:#5cd980,color:#bfffd0
```

- `MermaidPlugin` 在 [`theme/index.ts`](https://github.com/android-security-engineer/Vector-skills/blob/master/website/docs/.vitepress/theme/index.ts) 的 `enhanceApp` 中 `app.use` 注册；
- `mermaidConfig` 经 [`config.ts`](https://github.com/android-security-engineer/Vector-skills/blob/master/website/docs/.vitepress/config.ts) 注入到每个 mermaid 实例，统一字体与 `curve: 'basis'` 连线风格；
- 深浅色切换时 SVG 内联，颜色随 CSS 变量联动，无需重新渲染。

## logo 与 favicon

```ts
head: [
  ['meta', { name: 'theme-color', content: '#0B0E14' }],
  ['link', { rel: 'icon', type: 'image/svg+xml', href: '/Vector-skills/favicon.svg' }]
],
themeConfig: { logo: '/favicon.svg' }
```

- `logo` 路径相对 `public/`，不带 base 前缀（VitePress 自动处理）；
- `head` 中 `href` 需带 base 前缀；
- favicon 放 `website/docs/public/favicon.svg`。

## 自定义组件

`theme/index.ts` 注册全局组件，可在任意 markdown 中使用：

```ts
enhanceApp({ app }) {
  app.use(MermaidPlugin)
  app.component('InjectionDiagram', InjectionDiagram)
}
```

`InjectionDiagram.vue` 即注入链路可视化组件。新增组件放 `theme/` 目录并在此注册。

## 自定义域名

默认部署到 `<user>.github.io/Vector-skills/`。使用自定义域名时：

1. 在 `website/docs/public/` 放 `CNAME` 文件，内容为域名（如 `docs.vector.dev`）；
2. DNS 配置 CNAME 指向 `<user>.github.io`；
3. **改 `base` 为 `'/'`**，自定义域名根路径无需子路径前缀；
4. GitHub Pages 设置中验证域名。

> ⚠️ 切换自定义域名后，`head` 中 favicon 等硬编码 base 的 `href` 也需同步改为 `'/'`。

### 域名切换状态机

自定义域名迁移是个有状态的过程，`base` 与 DNS 必须同步推进，否则会出现资源 404：

```mermaid
stateDiagram-v2
    [*] --> 默认域名: base='/Vector-skills/'
    默认域名 --> 放CNAME: 在 public/ 放 CNAME
    改DNS --> 等待解析: CNAME 指向 user.github.io
    改base --> base='/'
    放CNAME --> 改base: GitHub 验证域名
    改base --> 改head: href 去 base 前缀
    等待解析 --> 生效: DNS 全球传播
    改head --> 生效
    生效 --> [*]: 自定义域名上线
    默认域名 --> 改DNS: 也可先改 DNS
```

状态迁移要点：

- **放CNAME** 与 **改base** 必须成对出现，只改其一会导致静态资源带多余前缀或域名验证失败；
- `base` 定义在 [`config.ts`](https://github.com/android-security-engineer/Vector-skills/blob/master/website/docs/.vitepress/config.ts) 顶层，由 VitePress 编译时注入到所有资源 URL；
- **改head** 指修正 `head` 数组里写死的 `/Vector-skills/favicon.svg` 这类 `href`，自定义域名下应改为 `/favicon.svg`。

## footer

```ts
footer: {
  message: '基于 GPL-3.0 协议开源',
  copyright: 'Vector Framework · 本站为社区教学文档'
}
```

`editLink.pattern` 指向仓库 edit URL，点击"在 GitHub 上编辑此页"直接跳转源文件。

## 相关

- 配置全貌见 [config.ts](https://github.com/android-security-engineer/Vector-skills/blob/master/website/docs/.vitepress/config.ts)
- 本地预览见 [local](./local)
- 部署排错见 [troubleshoot](./troubleshoot)
- Pages 部署见 [pages](./pages)
