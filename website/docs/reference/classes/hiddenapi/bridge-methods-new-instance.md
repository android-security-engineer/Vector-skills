...
`UserHandle` 的 `public UserHandle(int h)` 构造器在 SDK 中未导出（`UserHandle` 实例通常通过常量 `ALL`/`CURRENT` 获取，但 Vector 需要按任意 userId 构造）。bridge 把它收口为静态方法：
...
`public class ByteBufferDexClassLoader extends BaseDexClassLoader` —— 直接从内存 `ByteBuffer` 加载 DEX 的隐藏类加载器，无需落盘。它不是 `HiddenApiBridge` 的静态方法，而是一个**继承 hidden 基类的桥类**。
...
| 桥 | 目标 | 参数 | 用途 |
| :--- | :--- | :--- | :--- |
| `HiddenApiBridge.UserHandle(int)` | `new UserHandle(int)` | `int h` | 按任意 userId 构造 UserHandle |
| `new ByteBufferDexClassLoader(...)` | hidden `BaseDexClassLoader` 子类 | `ByteBuffer[]` / `String` / `ClassLoader` | 从内存加载 DEX |
