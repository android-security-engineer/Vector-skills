...
Vector 的 11 个 Gradle 模块按"注入引擎 → 守护进程 → 管理 App → 兼容层"分层协作。下图展示一次模块 hook 从注入到生效的关键路径：

```mermaid
flowchart LR
    subgraph install ["安装/启动层"]
        ML["📦 magisk-loader<br/>service.sh/customize.sh"]:::loader
        ZG["🧬 zygisk<br/>Module/main"]:::zygisk
    end
    subgraph runtime ["运行时层"]
        DM["🛰️ daemon<br/>VectorService"]:::daemon
        AS["daemon/ipc<br/>ApplicationService"]:::daemon
        MS["daemon/ipc<br/>ManagerService"]:::daemon
        XP["🔌 xposed<br/>VectorModuleManager"]:::xposed
        LG["📜 legacy<br/>XposedBridge"]:::legacy
    end
    subgraph app ["管理 App"]
        APPMGR["🟦 app<br/>ConfigManager/ScopeAdapter"]:::appmgr
    end
    subgraph native ["native 基座"]
        NT["⚙️ native<br/>HookBridge/Context"]:::native
        HA["👁️ hiddenapi<br/>HiddenApiBridge"]:::hiddenapi
    end

    ML -->|"unshare 拉起"| DM
    ZG -->|"fork 注入应用进程"| XP
    DM -->|"requestApplicationService"| AS
    AS -->|"getModulesList"| XP
    XP -->|"loadModule 实例化"| LG
    LG -->|"hook 调度"| NT
    NT -->|"ART hook"| LSPLANT["external<br/>LSPlant"]:::external
    HA -->|"绕过 hidden 限制"| DM
    APPMGR -->|"ILSPManagerService IPC"| MS

    classDef loader fill:#3a2a10,stroke:#e8a838,color:#ffd9b0
    classDef zygisk fill:#0e3a36,stroke:#3dd8c8,color:#bff5ec
    classDef daemon fill:#143a4a,stroke:#4fb3d8,color:#bfe6f5
    classDef xposed fill:#1a3a1a,stroke:#5cd980,color:#bfffd0
    classDef legacy fill:#1a2030,stroke:#6b7689,color:#cdd6e3
    classDef appmgr fill:#143a4a,stroke:#4fb3d8,color:#bfe6f5
    classDef native fill:#1a2030,stroke:#6b7689,color:#cdd6e3
    classDef hiddenapi fill:#3a2a10,stroke:#e8a838,color:#ffd9b0
    classDef external fill:#0e3a36,stroke:#3dd8c8,color:#bff5ec
```
